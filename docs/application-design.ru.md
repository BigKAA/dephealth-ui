...
- **AlertManager** — активные алерты по зависимостям (опционально; если не настроен, алерт-элементы UI скрыты)
...
Точки входа — это первые сервисы, принимающие внешний трафик в топологии (например, API gateways, фронтенд-бэкенды). Они отмечаются синим бейджем (⬇) на графе и меткой «Точка входа» в боковой панели.

**Как работают точки входа:**

- Точки входа определяются **явной меткой `isentry`** на метриках dephealth SDK — автоматического определения нет
- Задайте метку `isentry=yes` в dephealth SDK или переменную окружения `DEPHEALTH_ISENTRY=yes` в uniproxy
- Если ни у одного сервиса нет метки `isentry`, ни один узел не будет отмечен как точка входа
- Метка `isentry` включена в `group by` PromQL-запросов для передачи через запросы топологии

**Отображение на фронтенде:**

- Узлы-точки входа показывают синий бейдж ⬇ в правом нижнем углу
- Боковая панель отображает индикатор «Точка входа» для этих узлов
- Легенда графа содержит иконку точки входа

### Идентификация dependency-узлов

Dependency-узлы (нецелевые сервисы — базы данных, кэши, брокеры сообщений) используют составной формат ID:

- **ID узла:** `{имя_источника}/{имя_зависимости}` (например, `order-service/postgres-main`)
- **Label узла:** логическое имя зависимости (например, `postgres-main`, `ldap`, `redis`)
- **Поля `host` и `port`:** содержат фактический endpoint подключения, отображаются как вторая строка в UI

**Ключевое поведение:**

- Каждая пара `(source, dependency)` создаёт **отдельный** dependency-узел — дедупликации по `host:port` нет
- Если два сервиса зависят от одной базы данных (`host:port`), на графе будут отдельные узлы (например, `order-service/postgres-main` и `payment-api/postgres-main`)
- Если имя зависимости совпадает с именем известного сервиса, зависимость ссылается на узел этого сервиса (формируя связный граф «сервис → сервис»)

### Резолюция группировки dependency-узлов

Метки `namespace` и `group` описывают **отчитывающийся сервис**, а не цель.
Поэтому dependency-узлы резолвируют собственное размещение по следующему
приоритету (зарезервированные необязательные метки `dep_namespace` /
`dep_group` описывают расположение цели; см. [спецификацию
метрик](./METRICS.ru.md)):

1. **Единогласные явные `dep_namespace` / `dep_group`** по всем входящим рёбрам
   dependency-узла. Конфликтующие непустые значения игнорируются и попадают в
   `meta.warnings` ответа топологии.
2. **Namespace из FQDN `host`** (`<service>.<namespace>.svc[.<domain>]` →
   namespace; только для измерения namespace).
3. **Наследование от сервисов-источников**, когда все источники данной
   зависимости согласны на одно значение namespace/group.
4. **Иначе узел остаётся вне групп** (отображается вне любых групп).

Сервисы-цели не затрагиваются: если цель сама является отчитывающимся
сервисом, она сохраняет собственные метки `namespace`/`group`, а метки `dep_*`
на её входящих рёбрах игнорируются.

> **Историческая справка:** до появления поддержки `dep_namespace`/`dep_group`
> группа dependency-узла бралась из первого встреченного входящего ребра
> (недетерминированно для общих зависимостей). Теперь общие endpoints без
> явных меток отображаются вне групп вместо попадания в произвольную группу.
...
Поле `alerts.enabled` указывает, настроен ли AlertManager. При `false` фронтенд скрывает все алерт-элементы UI (см. [Опциональный AlertManager](#опциональный-alertmanager)).
...
datasources:
  prometheus:
    ...
    # password: "secret"
  alertmanager:
    url: "http://alertmanager.monitoring.svc:9093"  # опционально; оставьте пустым для отключения алертов
...
- Узлы-зависимости (Redis, PostgreSQL и т.д.) получают `group` только при резолвинге (явная метка `dep_group` или единогласное наследование) — без группы полоса не отображается
...
### Опциональный AlertManager

AlertManager является опциональным источником данных. Если `datasources.alertmanager.url` пуст (или не настроен), приложение работает без данных об алертах.

**Config API:** `GET /api/v1/config` возвращает `alerts.enabled: false`, если URL AlertManager не настроен, и `alerts.enabled: true`, если настроен.

**Поведение фронтенда при отключённом AlertManager (`alerts.enabled: false`):**

- Кнопка алертов в тулбаре визуально неактивна (полупрозрачная, `cursor: not-allowed`) с tooltip «Подключите AlertManager»
- Клик по неактивной кнопке алертов не открывает drawer
- Бейджи алертов не отображаются на узлах и рёбрах графа
- Секции алертов скрыты в боковых панелях узлов и рёбер
- Счётчики алертов скрыты в статус-баре
- Данные алертов не запрашиваются с сервера

При настроенном AlertManager (`alerts.enabled: true`) все функции алертов работают как описано в остальных разделах документа.

> **Примечание:** Исторические алерты (в Режиме истории) извлекаются из Prometheus через метрику `ALERTS`, а не из AlertManager. На режим истории эта настройка не влияет.
...
- Шкала времени: мажорные/минорные тики с форматированными метками времени (адаптивные интервалы от 10 мин до 14 дней в зависимости от диапазона); подавление перекрывающихся меток; адаптация к размеру окна через ResizeObserver
